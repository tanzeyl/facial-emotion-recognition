...
from newDataGenerator import FERPlusParameters, FERPlusReader, display_summary

emotion_table = {'happy': 0,
                 'surprise': 1,
                 'sadness': 2,
                 'anger': 3,
                 'disgust': 4,
...
train_folders = ['CK+Train']
valid_folders = ['CK+Valid']
test_folders = ['CK+Test']
...
def main(base_folder):
    ...
    print(base_folder + '/ck+_train_processed_images.npy')
    print(base_folder + '/ck+_train_processed_labels.npy')
    print(base_folder + '/ck+_test_processed_images.npy')
    print(base_folder + '/ck+_test_processed_labels.npy')
    print(base_folder + '/ck+_val_processed_images.npy')
    print(base_folder + '/ck+_val_processed_labels.npy')

    np.save(base_folder + '/ck+_train_processed_images.npy', train_processed_images)
    np.save(base_folder + '/ck+_train_processed_labels.npy', train_processed_labels)

    np.save(base_folder + '/ck+_test_processed_images.npy', test_processed_images)
    np.save(base_folder + '/ck+_test_processed_labels.npy', test_processed_labels)

    np.save(base_folder + '/ck+_val_processed_images.npy', val_processed_images)
    np.save(base_folder + '/ck+_val_processed_labels.npy', val_processed_labels)
